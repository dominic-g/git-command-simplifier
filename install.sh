...
# Copy the files
cp clone.sh pu.sh pull.sh setup-env.sh common_github.sh "$DEST_DIR/" || { printf "${RED}Failed to copy files${RESET}\n"; exit 1; }
...
chmod +x "$DEST_DIR/clone" "$DEST_DIR/push" "$DEST_DIR/pull" "$DEST_DIR/github_token" || { printf "${ORANGE}Failed to make symbolic links executable${RESET}\n"; printf "You will have to use the full script names as commands name\n"; }
